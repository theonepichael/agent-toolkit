...
`backlog-item.md` fix), the running orchestrator is still the old one. Restart
it in place instead of hand-closing and hand-resuming:

```bash
python3 ~/.claude/scripts/herdr_delegate.py restart --swarm <N> --prefix <prefix> [--run-id <runId>] [--model <model>]
```

It closes the orchestrator's tab (never the workers'), relaunches it, and
prompts it with `/backlog-item --swarm=N resume <runId> --prefix <prefix>` so
it picks up the same run's persisted state instead of starting fresh. The
runId comes from `--run-id` when given, otherwise from the newest persisted
state file matching the prefix; with nothing resumable the command refuses
rather than silently launching a fresh run — pass `--run-id` or use `launch`
explicitly when a fresh start is what you want.
