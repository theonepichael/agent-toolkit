...
---
1. Run `python3 ~/.agent-toolkit/scripts/gen_interfaces.py --check`. If it exits 1, stop and tell the user to regenerate `INTERFACES.md` first — an out-of-date file makes the map below untrustworthy. If it exits 3, a skill doc's shown command no longer matches its script's contract — say which, and suggest fixing that doc; the map itself is still current.
