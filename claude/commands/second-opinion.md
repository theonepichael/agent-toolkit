...
allowed-tools: [Read, Write, AskUserQuestion, "Bash(python3 ~/.agent-toolkit/scripts/second_opinion.py:*)", "Bash(python3 ~/.agent-toolkit/scripts/grill.py:*)"]
---
...
"Plans and deliverables get a path on record" backlog policy: both the plan's
file path and the critique-notes file path end up in a tracking item's
`related_files`, whether that means creating the item (offer first) or updating
an existing one.
...
available backend(s) fail (e.g. opencode's `adversary` agent errors out — check
with `opencode run --agent adversary --auto --format json <text> 2>&1` if that
