...
`--swarm`/`--swarm=N` token if present (note which one was given) — what
...
straight to the `--swarm[=N] mode` section instead — it does not take a
single-item target; `--swarm <slug>` is a usage error, ask the user whether
they meant `--auto <slug>`. A `--prefix <prefix>` tail (sent by
`herdr_delegate.py launch`) or a `resume <runId> --prefix <prefix>` tail (sent
by `herdr_delegate.py restart`) also selects `--swarm` mode and carries that
run's queue scope and/or resume directive — see that section's step 1.
