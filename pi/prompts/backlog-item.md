...
---
<!-- generated by agent-scripts/gen_skills.py — do not edit; edit templates/backlog_item_pi_native.md.tmpl for shared wording or gen_skills.py's CAPABILITY_TABLE / *_PARAMS tables for harness-specific wording, then regenerate -->

Work the named item to done, one step at a time. The invocation (or
`$ARGUMENTS`) holds the target: strip a leading `--auto` or
`--swarm`/`--swarm=N` or `--serial` token if present (note which one was given) — what
remains is the target item, a slug or an integer N. If `--auto` was given,
skip straight to the `--auto mode` section at the end of this file instead of
running the numbered steps live. If `--swarm`/`--swarm=N` was given, skip
straight to the queue-runner section instead — it does not take a
single-item target; `--swarm <slug>` is a usage error, ask the user whether
they meant `--auto <slug>`. If `--serial` was given, skip to that same
section in serial mode; it also requires a prefix and never takes a target.
A `--prefix <prefix>` tail (sent by
...
## 1. Resolve
Call the `dev_status` tool with `action: "show", slug: "$ARGUMENTS"` (or the
named `<slug|N>` target — works whether `$ARGUMENTS` is expanded from a prompt
template or given as an identifier: `show` accepts a real slug or a numeric
position, read-only except for the dead-claim sweep: a dead session's stale claim
on any in-progress item it sees is reverted to open, live claims are never
touched). Its response's `id` field is this item's real slug — use that
resolved slug for every remaining step below, never the raw identifier or
`$ARGUMENTS` again (every mutating `dev_status` action refuses a numeric slug
outright).
...
If `start` fails naming this machine's id file, call `dev_status` with
`action: "machine_id", repair: true` and retry; never edit the id file by hand.
...
merged into the repo's local default branch (merge ancestry is the
...
actually reached the default branch; planning-only items with no
...
Runs the READY queue through recursive workers, each in its own herdr tab and
running `/backlog-item --auto <slug>`. `--swarm=N` runs up to N concurrently
(default 3); `--serial` uses the same scheduler with `mode="serial"`, forces
concurrency one, and does not launch the next item until the prior worker's
...
1. Pick a `runId` for this invocation (e.g. a short timestamp-based slug)
   ...
   **Resume.** If the invocation carried `resume <runId> --prefix <prefix>`
   ...
   `swarm_poll` with that exact runId, prefix, and mode. `getOrInitState` loads the
   ...
   from being re-selected. Persisted mode is sticky: omission inherits it,
   while an explicit mismatch refuses. Spawn with the given prefix exactly as passed —
...
2. Loop: call `swarm_poll`. It blocks until at least one worker settles and
   ...
   - **`finished`** / **`timed_out`** / **`error`** — record the outcome for
     ...
     all three normally close that worker's tab and free its slot, and none is
     ever silently retried. In serial mode, a failed or inconclusive close is
     persisted as `teardown_ambiguous` and still occupies the sole slot; follow
     the tool's exact close-and-reconcile instruction, then poll again. Never
     start the next item merely because its predecessor reached a terminal
     item outcome. A genuinely freed slot is the cue to call
     `swarm_spawn` again with the same `runId`, `prefix`, and mode: that
     re-reads READY in stable topological dashboard order, so a worker that
     just unblocked two items causes the first newly READY item to be picked
     up without anyone naming it. The three are deliberately
...
3. Repeat steps 1 and 2 — spawn, poll, spawn again — until `swarm_spawn`
   ...
   drained, or the run ends with a live worker holding an open tab. In serial
   mode, `teardown_ambiguous` is likewise not drained. When the serial tool
   reports quiescence, include its fresh dashboard: descendants of a failed
   blocker remain visibly BLOCKED rather than disappearing from the digest.
