...
- `pi/prompts/{dashboard,recap,grill-me,backlog-item,make-skill,spec,standup,to-tickets}.md`
  ...
  command; `pi/skills/*/SKILL.md` binds `/skill:name` or semantic match). 7
  of the 8 render from a dedicated `templates/{name}_pi_native.md.tmpl`
  file for both `pi` and the
...
- `pi/settings.json` is a copy-once seed. It carries **no** `skills` key.
  ...
  default scan, not a replacement for it, so listing
