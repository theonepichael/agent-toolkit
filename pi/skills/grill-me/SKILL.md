...
---
<!-- generated by agent-scripts/gen_skills.py — do not edit; edit templates/grill_me_pi_native.md.tmpl for shared wording or gen_skills.py's CAPABILITY_TABLE / *_PARAMS tables for harness-specific wording, then regenerate -->
...
Also check the vitals store for already-settled facts on this topic before asking about it: use the `vitals_promotion` tool with action `search` and `query` set to the topic keywords (set `backlogSlug` too, if this session is tied to a backlog item) instead of reading the raw file. Treat a matching record as a settled fact: don't re-ask a question it already answers, and cite it (`source_slug`/`source_decision_id`) if it informs a new decision. Skip this check entirely if the vitals directory doesn't exist yet — nothing has been promoted. If the `vitals_promotion` tool returns an error, report it as an error — a failed search is an error to report, not 'no settled facts'.
