...
---
<!-- generated by agent-scripts/gen_skills.py — do not edit; edit templates/spec_pi_native.md.tmpl for shared wording or gen_skills.py's CAPABILITY_TABLE / *_PARAMS tables for harness-specific wording, then regenerate -->

Produce a complete implementation spec for the specified task.

If no task was named (or $ARGUMENTS is empty), use the task under discussion in the conversation; if neither exists, ask what to spec.

## 1. Draft the eight fields

Fill in what you can from the task description (or $ARGUMENTS) and context already in scope (files read, prior discussion) — don't ask about anything inferable from the codebase.
