...
This is a CLI/integration characterization suite for dev_status_impl.py's
command surface, not a collection of leftover unit tests for modules that
were later extracted from it -- that's why it stayed large even after
dev_status_storage.py, dev_status_formatting.py, dev_status_read.py, and
dev_status_types.py were pulled out: those extractions moved pure logic
out of dev_status_impl.py, and this file was never testing that logic in
...
class ThinLauncherTests(unittest.TestCase):
    ...
    def test_launcher_cli_matches_impl_cli(self):
        ...
        scripts = Path(__file__).resolve().parent.parent / "agent-scripts"
