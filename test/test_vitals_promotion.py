...
class CliPassTests(TempDirTestCase):
    ...
    def run_cli(self, *args: str) -> subprocess.CompletedProcess[str]:
        return subprocess.run(
            [
                sys.executable,
                str(Path(__file__).resolve().parent.parent / "agent-scripts" / "vitals_promotion.py"),
                "--data-dir",
                str(self.data_dir),
                *args,
            ],
    ...
    def test_help_description_matches_docstring_first_line(self) -> None:
        ...
        result = subprocess.run(
            [sys.executable, str(Path(__file__).resolve().parent.parent / "agent-scripts" / "vitals_promotion.py"), "--help"],
            capture_output=True,
            text=True,
            env=env,
        )
