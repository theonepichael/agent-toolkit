...
never shell out to `codex`/`agy`/`opencode`/`pi` directly. Three operations:
...
critique, and `probe` runs one trivial, cheap, text-only request per model in
the selected backend's pool (or its single override / default model) and
reports per-model availability as JSON — use it to check a pool's health before
committing to a multi-round rotation, and remember it calls the real models
once each, so pass `--backend` deliberately. It is single-round: one call, one
