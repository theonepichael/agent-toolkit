...
never shell out to `codex`/`agy`/`opencode`/`pi` directly. Two operations:
...
plan revision are your job, not the script's. By default, reviews are grounded
in the target codebase (`--dir` or current working directory) with read-only
tools enabled; use `--text-only` to opt out.
