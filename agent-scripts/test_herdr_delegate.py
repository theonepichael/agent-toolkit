...
The script is owned by this repo (dotfiles-side originals were deleted; see
sync_from_dotfiles.py's header), so its internals are tested here, in
process: a FakeHerdr stands in for the herdr binary, recording argv and
answering canned envelopes, so no subprocess is ever spawned and the
conftest sandbox needs no marking.

Covered: the CLI contract (help, usage errors), and the `restart`
subcommand's whole decision tree -- HERDR_ENV/prefix refusals,
resolution-before-close ordering, close-exactly-one-by-label, the
deregistration poll, zero-match/multi-match handling, runId discovery
(explicit wins; persisted `prefix` field first, legacy slug-scan fallback;
refusal rather than a fresh launch), and prompt-contract exact-match.
"""
