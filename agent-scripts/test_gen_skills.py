...
class RepoIdentityTests(unittest.TestCase):
    """Prove dotfiles' own generated output is unaffected by this repo's fix.

    gen_skills_params.py's per-(skill, harness) PARAMS dicts are plain
    module-level string literals, built once by calling edit_root()/
    symlink_cmd()/probe_add_dir() at import time -- patching
    gen_skills_params.REPO_IDENTITY after the module is already imported
    does nothing, since those calls already ran and their results are
    frozen into the dicts. Getting a genuinely different REPO_IDENTITY's
    output means patching repo_identity.REPO_IDENTITY (the value
    gen_skills_params.py's `from repo_identity import REPO_IDENTITY`
    reads) and reloading gen_skills_params so its module body re-executes
    against the new value.

    The asserted strings are the exact pre-fix literals every rewritten
    site in agent-toolkit carried before this fix (verified directly
    against git history during this fix), which is what dotfiles' own
    copy of this file has always rendered and must keep rendering.
    """

    def _render_with_repo_identity(
        self, identity: str, skill: str, harness: str
    ) -> str:
        import importlib

        import repo_identity

        original = repo_identity.REPO_IDENTITY
        repo_identity.REPO_IDENTITY = identity
        try:
            importlib.reload(gen_skills_params)
            template_text = (REPO_ROOT / gs.TEMPLATE_PATHS[skill]).read_text(
                encoding="utf-8"
            )
            params = gen_skills_params.SKILL_PARAMS[skill][harness]
            return gs.render_one(skill, harness, template_text, params)
        finally:
            repo_identity.REPO_IDENTITY = original
            importlib.reload(gen_skills_params)

    def test_dotfiles_identity_reproduces_pre_fix_make_skill_claude_text(
        self,
    ) -> None:
        text = self._render_with_repo_identity("dotfiles", "make-skill", "claude")
        self.assertIn(
            "Put it in `~/dotfiles/claude/commands/ref/<skill>-<topic>.md` and "
            'point to it from the step that needs it: "Read '
            '`~/.claude/commands/ref/...` when X."',
            text,
        )
        self.assertIn(
            "Pass `--add-dir ~/dotfiles` when the probe must read a skill file",
            text,
        )
        self.assertIn("1. File lives at `~/dotfiles/claude/commands/<name>.md`.", text)
        self.assertIn(
            "3. Create the live symlink now: "
            "`ln -s ~/dotfiles/claude/commands/<name>.md ~/.claude/commands/<name>.md`.",
            text,
        )
    ...
    def test_dotfiles_identity_reproduces_pre_fix_standup_claude_text(self) -> None:
        text = self._render_with_repo_identity("dotfiles", "standup", "claude")
        self.assertIn(
            "See `~/dotfiles/claude/scripts/standup_adapters.py`\nfor wiring "
            "in a real adapter",
            text,
        )

    def test_agent_toolkit_identity_matches_committed_output(self) -> None:
        """After the reload dance above, the real REPO_IDENTITY must still
        produce exactly what's committed -- guards against the reload
        leaking the wrong identity into later tests or a real run."""
        text = self._render_with_repo_identity("agent-toolkit", "make-skill", "claude")
        on_disk = (REPO_ROOT / gs.OUTPUT_PATHS[("make-skill", "claude")]).read_text(
            encoding="utf-8"
        )
