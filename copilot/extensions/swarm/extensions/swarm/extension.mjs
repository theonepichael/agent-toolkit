...
// copilot/extensions/swarm/src/extension.ts
var context = new SwarmToolContext();
await joinSession({
  tools: [
    {
      name: "swarm_spawn",
      description: "Spawn workers for READY backlog items in concurrent or serial mode, one herdr tab each.",
      parameters: {
        type: "object",
        properties: {
          runId: {
            type: "string",
            description: "Identifier for this swarm run -- reused across spawn/poll/resolve calls."
          },
          items: {
            type: "array",
            items: { type: "string" },
            description: "Backlog item slugs to spawn. Omit to select automatically from READY queue via prefix."
          },
          prefix: {
            type: "string",
          ...
          mode: {
            ...
            description: "Sticky execution mode for this run. Serial forces one worker and uses serial_safe eligibility."
          },
          model: {
            type: "string",
    ...
    {
      name: "swarm_poll",
      description: "Wait for at least one active swarm worker to settle or check in, returning events.",
      parameters: {
        type: "object",
        properties: {
          runId: {
          ...
          relayStallMs: {
            type: "number",
            description: "How long a worker may sit awaiting relay before flagging stall."
          },
          workerDeadlineMs: {
            type: "number",
    ...
    {
      name: "swarm_amend",
      description: "Tell a running worker its backlog item has been corrected, so it re-reads the item before continuing.",
      parameters: {
        type: "object",
        properties: {
          runId: {
