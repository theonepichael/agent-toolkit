...
It closes the orchestrator's tab (never the workers'), relaunches it, and prompts it with `/backlog-item --swarm=N resume <runId> --prefix <prefix>` so it picks up the same run's persisted state instead of starting fresh. The runId comes from `--run-id` when given, otherwise from the newest persisted state file matching the prefix; with nothing resumable the command refuses rather than silently launching a fresh run — pass `--run-id` or use `launch` explicitly when a fresh start is what you want.
...
`pi/prompts/backlog-item.md`'s or Copilot's `/backlog-item` `--swarm[=N]` section
