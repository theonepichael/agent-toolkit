...
If the invoking message names an explicit file path or inline plan text (e.g.
another command or skill delegated to you with a specific plan), treat it as
the target. Otherwise infer it: prefer `grill.py show`'s most recent session
...
overwriting it — `question` tool: `Yes, overwrite (recommended)` / `No, leave
as-is`. Never silently rewrite a file. The critique-notes file is new each run,
so it doesn't need the same overwrite confirmation.
...
Then `question` tool: `Keep the current approach (recommended)` / `Use the
reviewer's suggestion` / `Let me decide manually`. Never silently pick a side
when the round cap is hit mid-disagreement.
...
happens). Relay that message and stop — don't fall back to critiquing the plan
